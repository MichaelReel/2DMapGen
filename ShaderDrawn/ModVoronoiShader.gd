...
func _ready():
	...
	material.set_shader_param("PointData", _get_data_texture())
	material.set_shader_param("PointColor", _get_color_texture())
...
func _get_color_texture() -> ImageTexture:
	var data := ImageTexture.new()
	var img := Image.new()
	var data_points := StreamPeerBuffer.new()
